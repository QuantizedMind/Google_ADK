...
# The user must specify whether to start `server1.py`, `server2.py`, or `server3.py`
...
#   python main.py --server server3 --log-level WARNING
# ------------------------------------------------------------------
...
from server3 import main as server3_main

# --------------------------------------------------
# Define a CLI entry point using click.
# The `--server` option is required and must be one of the three valid choices.
# The `--log-level` option is optional and passed through to the target server.
# --------------------------------------------------
@click.command()
@click.option(
    "--server",
    type=click.Choice(["server1", "server2", "server3"]),
    required=True,
    help="Select which server to run (server1, server2, or server3)",
)
@click.option(
    "--log-level",
    default="DEBUG",
...
def main(server: str, log_level: str):
    ...
    if server == "server1":
        ...
        sys.exit(server3_main(args=args, standalone_mode=False))
    else:
        print("❌ Invalid server option. Use --server with either 'server1', 'server2', or 'server3'.")
        sys.exit(1)
